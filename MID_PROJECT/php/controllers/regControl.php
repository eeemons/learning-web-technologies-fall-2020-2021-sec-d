<?php
    ...
    if(isset($_POST['submit'])){
        ...
        elseif($name>!'a'&& $name<!'z' || $name>!'A' && $name<!'Z'){
            header('location:../registration.php?error=alphabet_error');
        }
        else if(strlen($password) < 5){
            header('location:../registration.php?error=password_error');

        }
        else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
            header('location:../registration.php?error=email_error');
        }
        else{
            ...
            header('location:../registration.php?error=false');
        }
    }
